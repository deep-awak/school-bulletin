...
public class AdminBootstrapService {
	...
	public void initBootstrapConfigTable() {
		try {
			String createTableSql = "CREATE TABLE IF NOT EXISTS bootstrap_config (" +
	...
	public void bootstrapAdminIfNeeded() {
		boolean alreadyBootstrapped = bootstrapConfigRepository.existsByKey(BOOTSTRAP_KEY)
				&& "true".equals(bootstrapConfigRepository.findByKey(BOOTSTRAP_KEY)
		...
		if (adminExists) {
			...
			bootstrapConfigRepository.findByKey(BOOTSTRAP_KEY)
					.ifPresent(config -> {
		...
		bootstrapConfigRepository.findByKey(BOOTSTRAP_KEY)
				.ifPresentOrElse(
						...
						() -> {
							BootstrapConfigEntity newConfig = BootstrapConfigEntity.builder()
									.key(BOOTSTRAP_KEY)
									.value("true")
